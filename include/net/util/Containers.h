  ...
  namespace util {
    ...
    namespace extract_container_keys {
      template <typename Container, typename Enable = void> struct const_iterator {
        ...
        typedef typename KeyType<Container>::Type key_type;
    ...
    template <typename Container> struct ExtractContainerKeys {
      ...
      Container const& extractFrom;
      ExtractContainerKeys(Container const& extractFrom)
        : extractFrom(extractFrom) {
      }
      size_type size() const {
        return extractFrom.size();
      }
      const_iterator begin() const {
        return const_iterator(0,extractFrom.begin());
      }
      const_iterator end() const {
        return const_iterator(extractFrom.size(),extractFrom.end());
      }
    };
