...
/* NOTE: This is a copy of the klee/main.cpp as of 2011-12-03
 * I considered the matter quite a while and decided this was the best solution.
 * The problem is, that we need to create our own Interpreter and InterpreterHandler,
 * but want to keep pretty much everything else just like Klee.
 * As the klee infrastructure does not allow us to solve this with subclassing
 * (Interpreter::create is carved in stone!) we have to do it the ugly way :(
 */
...
#include "klee/Expr.h"
//#include "klee/Interpreter.h" This is replaced by kleenet::Interpreter
...
class KleeHandler : public kleenet::InterpreterHandler {
private:
...
public:
  ...
  void logClusterChange(std::set<unsigned> const& currentClusters) {
    ...
    if (DumpClusterChanges) {
      llvm::outs() << "[" << diff << "s] Clusterchange: Now " << currentClusters.size() << " clusters in total.";
...
int main(int argc, char **argv, char **envp) {
  ...
  Interpreter *interpreter = 
    theInterpreter = kleenet::NetExecutorBuilder::create(context, IOpts, handler); // FIXME
