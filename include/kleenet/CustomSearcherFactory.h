...
namespace kleenet { // implemented for the KleeNet project
  class Searcher;
  ...
  class CustomSearcherFactory {
    ...
    protected:
      ...
      virtual Searcher* createSearcher(net::PacketCacheBase*) = 0;
    public:
      static Searcher* attemptConstruction(Precedence, net::PacketCacheBase*);
  };
  ...
  template <typename S, typename Condition, bool autoNew = true> class CustomSearcherAutoFactory : public CustomSearcherFactory {
    ...
    protected:
      ...
      Searcher* createSearcher(net::PacketCacheBase* pcb) {
        if (condition) {
          net::Searcher* const ns = newSearcher(pcb);
          if (ns) {
            return new Searcher(std::auto_ptr<net::Searcher>(ns));
          }
        }
        return NULL;
      }
